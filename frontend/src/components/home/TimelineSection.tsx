...
export default function TimelineSection() {
  ...
  useEffect(() => {
    const fetchTimelineEvents = async () => {
      try {
        const timelineData = await api.timeline.getAll();
        ...
        // Fallback to default timeline events
        setTimelineEvents([
          {
            id: 1,
            year: "2024",
  ...
  if (loading) {
    return (
      <section id="timeline" className="w-full relative flex flex-col items-center bg-white text-gray-900 py-20">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-16">
            <p className="text-lg mb-4 text-gray-600 bg-gray-100 text-[12px] inline-block px-4 py-2 rounded-full">
              Timeline
            </p>
            <h2 className="text-3xl md:text-6xl font-bold mb-4">Our Journey</h2>
            <p className="text-lg text-gray-600">Milestones in IoT Innovation</p>
          </div>
                ...
                {event.side === "left" ? (
                  <>
                    <div className="w-1/2 pr-8 text-right">
                      <div className={`bg-white p-6 rounded-2xl shadow-lg border border-gray-200 hover:shadow-xl transition-all duration-300 hover:scale-105 ${event.is_future ? 'bg-gradient-to-br from-[#75BF43]/5 to-[#5a9f33]/5 border-[#75BF43]/20' : ''}`}>
                    ...
                    <div className={`absolute left-1/2 transform -translate-x-1/2 w-4 h-4 bg-[#75BF43] rounded-full border-4 border-white shadow-lg z-10 ${event.is_future ? 'animate-pulse bg-gradient-to-r from-[#75BF43] to-[#5a9f33]' : ''}`}></div>
          ...
          <div className="space-y-6">
            {timelineEvents.map((event, index) => (
              <div key={index} className="flex items-start space-x-4" data-aos="fade-up" data-aos-delay={index * 100}>
                <div className={`w-4 h-4 bg-[#75BF43] rounded-full mt-2 flex-shrink-0 ${event.is_future ? 'animate-pulse' : ''}`}></div>
                <div className={`bg-white p-4 rounded-xl shadow-lg border border-gray-200 flex-1 ${event.is_future ? 'bg-gradient-to-br from-[#75BF43]/5 to-[#5a9f33]/5 border-[#75BF43]/20' : ''}`}>
